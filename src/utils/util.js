...
export const formatDateForTimeline = (date, t) => {
  const n = t || 24 * 7; // 如果 t 未提供，默认生成一周（7 天）每天 24 小时的数据
  const D = date ? new Date(date) : new Date('2018/03/01'); // 如果 date 未提供，默认使用 2018/03/01

  const time = []; // 存储时间戳的数组
  const week = []; // 存储日期（MM/DD 格式）的数组

  let currentDate = new Date(D.getFullYear(), D.getMonth(), D.getDate()); // 获取传入日期的起始时间
  for (let i = 0; i < n; i++) {
    const timestamp = currentDate.getTime(); // 获取当前时间的时间戳
    time.push(timestamp); // 将时间戳添加到 time 数组中

    if (i % 24 === 0) { // 每 24 小时（即一天）记录一次日期
      week.push(moment(currentDate).format('MM/DD'));
    }

    currentDate.setHours(currentDate.getHours() + 1); // 将时间推进 1 小时
  }

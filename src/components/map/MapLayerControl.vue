  ...
  <div class="map-layer" :class="{'map-layer-phone': isMobile}"  :style="{'margin-top': (!showMoreFlag ? marginTop :moreMarginTop) }" >
    ...
    <div class="map-tips">
      ...
      <div class="lat-lng-tips" :class="{'lat-lng-tips-phone': isMobile}">
        <span>Latitude:<b>{{currentLat[0]}}°{{currentLat[1]}}'{{currentLat[2]}}"</b></span>
        <span>Longitude:<b>{{currentLng[0]}}°{{currentLng[1]}}'{{currentLng[2]}}"</b></span>
      </div>
    ...
    <!--    Base layer  -->
      ...
      <ul class="layer-control-ul">
        <li :class="{'active': showMoreFlag}" @click="toggleMoreLayer()"><i class="el-icon-plus"  ></i><span v-show="!isMobile">More</span></li>
      </ul>
      <!-- Device layer -->
    ...
    <!-- pressure level -->
    <div class="pressure-layer-control" :class="{'pressure-layer-control-phone': isMobile}">
      <ul class="layer-control-ul">
        <li @click="togglePressureLayer()">
          <i><img :src="planeIcon" alt=""/></i>
          <span v-show="!isMobile">气压层</span>
        </li>
        <li v-for="(item, index) in pressureLayers" :key="index" @click="selectPressureLayer(item)" v-show="showPressureOptions" :class="{'active' : item.active}">
...
<script>
  ...
  import playIcon from '@/assets/map/play.png'
  import pauseIcon from '@/assets/map/stop.png'
  ...
  export default {
    ...
    data () {
      return {
        historyDate:new Date('2018/9/15'),
        isMobile: isMobile(),
        playIcon:playIcon,
        planeIcon:planeIcon,
        pauseIcon:pauseIcon,
        ...
        r : 0,
        baseMapLayerLegend:[
          ['m/s',0,3,5,10,15,20,30,40,50,60],
          // ['m',0.5,1,2,3,5,7,9],
          ...
          // ['KT',0,45,60,80]
          // ['°C',16,18,20,22,23,24,25,26,27,28,29,30,32],
          ['°C',10,15,18,22,26,29,33]
        ...
        timeValue: [0],
        timeLineData: formatDateForTimeline('2018/9/15'),
        timeLineMax:168,
        timeFlag:false,
        ...
        pickerOptions: {
          disabledDate(time) {
            return time.getTime() < new Date('2016/1/1').getTime() || time.getTime() > new Date('2025/2/15').getTime();
          },
        },
        ...
        showPressureOptions: false,
        pressureLayers: [
          { layerName: 'surface', active: false, pressureLevel: '1000' },
          { layerName: '950hpa', active: false, pressureLevel: '950' },
          { layerName: '925hpa', active: false, pressureLevel: '925' },
          { layerName: '850hpa', active: false, pressureLevel: '850' },
          { layerName: '700hpa', active: false, pressureLevel: '700' }
        ],
        selectedPressureLayer: null
      }
    },
    mounted(){
      this.$nextTick(() =>{
    ...
    methods:{
      togglePressureLayer() {
        this.showPressureOptions = !this.showPressureOptions;
      },
      selectPressureLayer(item) {
        this.pressureLayers.forEach(layer => {
          layer.active = layer.layerName === item.layerName;
        });
        this.selectedPressureLayer = item.pressureLevel;
        this.inputTimeLine(this.timeValue, this.baseLayerIndex, this.selectedPressureLayer);
        console.log('pressure level now',this.selectedPressureLayer);
      },

      changeHistoryDate(e) {
        this.timeLineData = formatDateForTimeline(new Date(e));
        console.log("Selected timestamp:", e);
        console.log('timeLineData.time:', this.timeLineData.time);
        this.timeValue = 0;
        this.inputTimeLine(this.timeValue);
        this.stopTimeLine();
        console.log(this.timeLineData);
        if (this.timeLineData && this.timeLineData.time) {
          this.timeLineMax = this.timeLineData.time.length - 1;
        } else {
          console.error('timeLineData.time is not available');
          this.timeLineMax = 0;
        }
      },
      // 切换 picker 图标
      ...
      showTimelineTips(){
        $('.el-slider__runway').mousemove( (e) => {
          ...
          let n = (lineWdith / this.timeLineMax).toFixed(2)
          ...
          // console.log('===> sssss',this.customTips)
      ...
      // 检查 v 是否在合法范围内
      if (v >= 0 && v < this.timeLineData.time.length) {
        const timestamp = +this.timeLineData.time[v];
        // 检查转换后的时间戳是否为有效数字
        if (!isNaN(timestamp)) {
          return moment(timestamp).format('MM/DD HH:00');
        } else {
          console.error('Invalid timestamp in timeLineData.time:', this.timeLineData.time[v]);
          return 'Invalid Date';
        }
      } else {
        console.error('Index out of range for timeLineData.time:', v);
        return 'Invalid Date';
      }},
      ...
      inputTimeLine(v, i, pressureLevel) {
        console.log(v);
        let { time } = this.timeLineData;
        let index = i != null ? i : this.baseLayerIndex;
        let item = this.baseLayerV1[index];
        console.log('===> 自动调用', item, v, time[v]);

        // 创建 payload 对象并添加气压层信息
        const payload = {
          flag: false,
          time: time[v],
          index: index,
          pressureLevel: pressureLevel || '950' 
        };

        this.$store.dispatch('moduleMap/' + item.layerType, payload);
    },
      // 播放时间
      playTimeLine(){
        let tt =  this.baseLayerIndex == 0 || this.baseLayerIndex == 3  ? 4000 : 4000
        this.timeFlag = true
        this._t = setInterval(()=>{
...
<style scoped lang="less">
  ...
  .pressure-layer-control-phone {
    right: 10px;
    top: 10px;
    zoom: 0.88;
  }

  .pressure-layer-control .layer-control-ul {
    list-style: none;
    padding: 0;
    margin: 0;
  }

  .pressure-layer-control .layer-control-ul li {
    cursor: pointer;
    font-size: 14px;
    width: 120px;
    height: 30px;
  ...
  .pressure-layer-control .layer-control-ul li.active i {
    background-color: #00a0e9;
    box-shadow: #333333 0px 2px 2px;
  }

  .pressure-layer-control .layer-control-ul li.active span {
    background-color: rgba(2, 2, 2, 0.5);
  }

  .pressure-layer-control .layer-control-ul li:hover i {
    transition: all .3s;
    background-color: #e60012;
    box-shadow: #333333 0px 2px 2px;
  }

  .pressure-layer-control .layer-control-ul li:hover span {
    transition: all .3s;
    background-color: rgba(2, 2, 2, 0.2);
  }

  .pressure-layer-control .layer-control-ul li i {
    width: 30px;
    height: 30px;
    border-radius: 50%;
    display: flex;
  ...
  .pressure-layer-control .layer-control-ul li i img {
    height: 90%;
  }

  .pressure-layer-control .layer-control-ul li span {
    font-weight: 400;
    margin-left: 10px;
    padding: 2px 10px;
    border-radius: 20px;

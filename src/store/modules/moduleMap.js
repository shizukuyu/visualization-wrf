...
import windIcon from "@/assets/map/Windground.png";
import temperatureIcon from "@/assets/map/temperature.png";
import waveIcon from "@/assets/map/wave.png";
import seaTemperatureIcon from "@/assets/map/Inlet_water_temperature.png";
import oceanCurrentIcon from "@/assets/map/current.png";
import airPressureIcon from "@/assets/map/pressure.png";
import salinityIcon from "@/assets/map/salinity.png";
// import shipIcon from "@/assets/map/船舶.png";
// import pipelineIcon from "@/assets/map/管线.png";
import portIcon from "@/assets/map/port.png";
// import libraryIcon from "@/assets/map/仓库.png";
import latlonIcon from "@/assets/map/latlongrid.png";
...
export default {
  ...
  state: {
    ...
    map:{
      titleLayers:[
        ...
        {url:'http://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}'}, // 智图深蓝色
  ...
  actions: {
    ...
    //     let d1 = http.get(getWindInfo,{time:t || 1519884000000 })  // 全球数据
    ...
    async setWindDataV1(context,payload){
      console.log('===>', payload);
      if (!payload.flag) {
        context.commit('SET_LAYER_FLAG', false);
        ...
        const pressureLevel = payload.pressureLevel || '950'; // 默认使用 950hpa
        const fileName = `wind_${year}-${month}-${day}${hour}_${pressureLevel}.json`;

        const requestUrl = `/json/${fileName}`;
        console.log('尝试请求的文件 URL:', requestUrl);
        ...
        try {
            ...
            if (response.ok) {
                ...
                console.log('获取到的 JSON 数据', payload.data);
            } else {
                throw new Error(`Failed to fetch ${fileName}`);
            }
        } catch (error) {
            console.error(`未找到匹配的文件: ${fileName}`, error);
            payload.data = null;
        }
        console.log('获取到的payload数据',payload);
        context.commit('SET_BASE_LAYER_INDEX',payload)
        context.commit('SET_WIND_DATA', payload);
        context.commit('SET_LAYER_FLAG', true);
      } else {
    ...
    setAirTemperatureData(context,payload){
      if (!payload.flag){
        context.commit('SET_LAYER_FLAG',false)
        let t = Number(payload.time)
        let d1 = http.get(getAirTemperatureInfo, { time: t || 1519884000000 });
            Promise.all([d1]).then((value) => {
                console.log('Promise==>', value);
                if (!checkResponseData(value)) {
                  context.commit('SET_LAYER_FLAG',true)
    ...
    // setAirTemperatureDataV1(context,payload){
    //       if (!payload.flag){
    //         context.commit('SET_LAYER_FLAG',false)
    //         context.commit('SET_BASE_LAYER_INDEX',payload)
    ...
    async setAirTemperatureDataV1(context,payload){
      console.log('===>', payload);
      if (!payload.flag) {
        context.commit('SET_LAYER_FLAG', false);
        context.commit('SET_BASE_LAYER_INDEX',payload)
        ...
        const day = String(timestamp.getDate()).padStart(2, '0');
        const hour = String(timestamp.getHours()).padStart(2, '0');
        const fileName = `T2_${year}-${month}-${day}${hour}.json`;

        // 修改请求 URL，匹配实际文件路径
        const requestUrl = `/json-t2/${fileName}`;
